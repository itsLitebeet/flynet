...
async def required_channel_id(db: Database, settings: Settings) -> int | None:
    """Active channel id. Env applies only until admin explicitly disables the gate."""
    if await db.is_required_channel_turned_off():
        return None
    cid = await db.get_required_channel_id()
    if cid is not None:
        return cid
    return settings.required_channel_id


async def is_gate_enabled(db: Database, settings: Settings) -> bool:
    return (await required_channel_id(db, settings)) is not None
...
async def check_user_membership(
    ...
    channel_id = await required_channel_id(db, settings)
...
async def resolve_join_url(
    ...
    stored = await db.get_required_channel_link()
    if stored:
        return stored
    channel_id = await required_channel_id(db, settings)
    if channel_id is None:
...
async def channel_label(db: Database, settings: Settings) -> str:
    title = await db.get_required_channel_title()
    if title:
        return escape(title)
...
async def join_prompt_text(bot: Bot, db: Database, settings: Settings) -> str:
    body = texts.JOIN_CHANNEL_REQUIRED.format(channel=await channel_label(db, settings))
...
async def save_invite_link(db: Database, raw: str) -> tuple[bool, str]:
    link = _normalize_invite_link(raw)
    if link is None:
        return False, texts.REQ_CHANNEL_LINK_INVALID
    if (await db.get_required_channel_id()) is None:
        return False, texts.REQ_CHANNEL_LINK_NO_CHANNEL
    await db.set_required_channel_link(link)
