...
async def get_role(user_id: int, settings: Settings, db: Database) -> str:
    if is_owner(user_id, settings):
        return ROLE_OWNER
    return await db.get_admin_role(user_id)


async def permissions_for(user_id: int, settings: Settings, db: Database) -> frozenset[str]:
    from app.role_permissions import permissions_for_role

    role = await get_role(user_id, settings, db)
    return await permissions_for_role(db, role)
...
async def has_permission(
    ...
    if is_owner(user_id, settings):
        return True
    return perm in (await permissions_for(user_id, settings, db))


async def can_access_panel(user_id: int, settings: Settings, db: Database) -> bool:
    return await has_permission(user_id, PANEL, settings, db)
