...
class UserMiddleware(BaseMiddleware):
    ...
    async def __call__(
        ...
        if tg_user is not None and not tg_user.is_bot:
            ...
            if (await self.db.is_banned(tg_user.id)) and not is_staff(
...
class ChannelJoinMiddleware(BaseMiddleware):
    ...
    async def __call__(
        ...
        if not await is_gate_enabled(self.db, self.settings):
            return await handler(event, data)
        ...
        channel_id = await required_channel_id(self.db, self.settings)
