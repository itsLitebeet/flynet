...
async def _buyer_keyboard_for(user_id: int | None, db: Database):
    show_test = user_id is not None and await buyer_show_test_button(db, user_id)
...
async def cb_cancel_support(
    ...
    if callback.message is not None:
        ...
        await callback.message.answer(
            texts.CANCELLED, reply_markup=await _buyer_keyboard_for(user_id, db)
        )
    await callback.answer()
...
async def cmd_cancel_support(
    ...
    await message.answer(texts.CANCELLED, reply_markup=await _buyer_keyboard_for(user_id, db))
...
async def on_support_message(
    ...
    ticket_id = await db.create_ticket(user_id=user.id, message=text)
    await state.clear()
    await message.answer(
        texts.SUPPORT_SENT, reply_markup=await _buyer_keyboard_for(user.id, db)
    )
...
async def on_support_reply(
    ...
    try:
        ...
        return
    await db.create_ticket_reply(ticket_id=ticket_id, sender="admin", message=reply)
    await db.close_ticket(ticket_id)
    await state.clear()
    await message.answer(
        texts.SUPPORT_REPLY_SENT_ADMIN,
        reply_markup=await keyboards.admin_reply_keyboard(message.from_user.id, settings, db)
