...
CREATE TABLE IF NOT EXISTS orders (
    ...
    traffic_warning_sent INTEGER NOT NULL DEFAULT 0,
...
CREATE INDEX IF NOT EXISTS idx_service_packages_loc ON service_packages(location_id);
...
class Database:
    ...
    @classmethod
    async def create(cls, path: Path | str) -> Database:
        ...
        await self._backfill_location_pricing()
        return self
    ...
    async def _ensure_column(self, table: str, column: str, sql_type: str) -> None:
        ...
        async with self._cursor() as cur:
            ...
            if column not in cols:
                await cur.execute(f"ALTER TABLE {table} ADD COLUMN {column} {sql_type}")
    ...
    async def _migrate(self) -> None:
        ...
        """
        await self._ensure_column("locations", "sub_url_template", "TEXT")
        await self._ensure_column("locations", "config_buttons", "TEXT DEFAULT '[]'")
        await self._ensure_column("locations", "is_test", "INTEGER NOT NULL DEFAULT 0")
        await self._ensure_column("locations", "price_base", "INTEGER")
        await self._ensure_column("locations", "price_per_gb", "INTEGER")
        await self._ensure_column("locations", "price_per_day", "INTEGER")
        await self._ensure_column("orders", "nickname", "TEXT")
        await self._ensure_column("orders", "admin_receipt_refs", "TEXT")
        await self._ensure_column("locations", "is_test", "INTEGER NOT NULL DEFAULT 0")
        await self._ensure_column(
            "locations", "purchase_enabled", "INTEGER NOT NULL DEFAULT 1"
        )
        await self._ensure_column("orders", "is_test", "INTEGER NOT NULL DEFAULT 0")
        await self._ensure_column(
            "orders", "admin_manual_only", "INTEGER NOT NULL DEFAULT 0"
        )
        await self._ensure_column("orders", "renew_of_order_id", "INTEGER")
        await self._ensure_column("orders", "traffic_warning_sent", "INTEGER NOT NULL DEFAULT 0")
        await self._ensure_column("orders", "expiry_warning_sent", "INTEGER NOT NULL DEFAULT 0")
        # Legacy status from an earlier version — hard-delete on upgrade.
        async with self._cursor() as cur:
            await cur.execute("DELETE FROM orders WHERE status = 'panel_removed'")
        ...
        await self._migrate_required_channel_settings()

    async def _migrate_required_channel_settings(self) -> None:
        async with self._cursor() as cur:
            await cur.execute(
                "INSERT OR IGNORE INTO settings (key, value) VALUES (?, ?)",
                (SETTING_REQUIRED_CHANNEL_OFF, "0"),
            )
        if await self.get_required_channel_id() is not None:
            return
        if await self.is_required_channel_turned_off():
            return
        if await self.get_required_channel_title() or await self.get_required_channel_link():
            await self.set_setting(SETTING_REQUIRED_CHANNEL_OFF, "1")

    async def _load_admin_roles(self) -> dict[str, str]:
        raw = await self.get_setting(SETTING_ADMIN_ROLES, "{}") or "{}"
        try:
            data = json.loads(raw)
    ...
    async def _save_admin_roles(self, roles: dict[str, str]) -> None:
        await self.set_setting(SETTING_ADMIN_ROLES, json.dumps(roles, ensure_ascii=False))

    async def get_admin_role(self, user_id: int) -> str:
        roles = await self._load_admin_roles()
    ...
    async def set_admin_role(self, user_id: int, role: str) -> None:
        ...
        await self._save_admin_roles(roles)

    async def list_staff_roles(self, staff_ids: list[int]) -> list[tuple[int, str]]:
        roles = await self._load_admin_roles()
    ...
    async def _backfill_location_pricing(self) -> None:
        base, per_gb, per_day = await self.get_pricing()
        async with self._cursor() as cur:
            await cur.execute(
    ...
    async def set_user_banned(self, user_id: int, banned: bool) -> bool:
        async with self._cursor() as cur:
            await cur.execute(
    ...
    async def list_users_paginated(self, offset: int, limit: int) -> list[sqlite3.Row]:
        async with self._cursor() as cur:
            await cur.execute(
    ...
    async def search_customers(self, query: str, *, limit: int = 15) -> list[sqlite3.Row]:
    ...
    async def get_pricing(self) -> tuple[int, int, int]:
        """Global default pricing (base, per_gb, per_day) in toman."""
        return (
            await self.get_int_setting("price_base", 0),
            await self.get_int_setting("price_per_gb", 0),
            await self.get_int_setting("price_per_day", 0),
        )

    async def _get_int_list_setting(self, key: str, fallback: list[int]) -> list[int]:
        raw = await self.get_setting(key)
        if not raw:
    ...
    async def _set_int_list_setting(self, key: str, values: list[int]) -> None:
        clean = sorted({int(v) for v in values if int(v) > 0})
        await self.set_setting(key, json.dumps(clean))

    async def get_volume_presets(self) -> list[int]:
        from app import texts

        return await self._get_int_list_setting(
            SETTING_VOLUME_PRESETS, texts.DEFAULT_VOLUME_PRESETS_GB
        )

    async def get_duration_presets(self) -> list[int]:
        from app import texts
    ...
    async def add_volume_preset(self, gb: int) -> tuple[bool, str]:
        if gb < 1 or gb > 500:
            return False, "invalid"
        presets = await self.get_volume_presets()
        if gb in presets:
            return False, "exists"
        if len(presets) >= MAX_PLAN_PRESETS:
            return False, "max"
        presets.append(gb)
        await self._set_int_list_setting(SETTING_VOLUME_PRESETS, presets)
        return True, "ok"

    async def remove_volume_preset(self, gb: int) -> tuple[bool, str]:
        presets = await self.get_volume_presets()
    ...
    async def add_duration_preset(self, days: int) -> tuple[bool, str]:
        if days < 1 or days > 3650:
            return False, "invalid"
        presets = await self.get_duration_presets()
        if days in presets:
            return False, "exists"
        if len(presets) >= MAX_PLAN_PRESETS:
            return False, "max"
        presets.append(days)
        await self._set_int_list_setting(SETTING_DURATION_PRESETS, presets)
        return True, "ok"

    async def is_manual_purchase_enabled(self) -> bool:
        return await self.get_setting(SETTING_MANUAL_PURCHASE, "0") == "1"

    async def set_manual_purchase_enabled(self, enabled: bool) -> None:
        await self.set_setting(SETTING_MANUAL_PURCHASE, "1" if enabled else "0")

    async def get_offer_config(self):
        from app.pricing import OfferConfig

        enabled = await self.get_setting(SETTING_OFFER_ENABLED, "0") == "1"
        kind = (await self.get_setting(SETTING_OFFER_KIND, "none") or "none").strip()
        if kind not in ("percent", "amount", "fixed"):
            kind = "none"
        value = await self.get_int_setting(SETTING_OFFER_VALUE, 0)
        if not enabled or kind == "none" or value <= 0:
            return OfferConfig(False, "none", 0)
        return OfferConfig(True, kind, value)

    async def set_global_offer(self, kind: str, value: int) -> None:
        if kind not in ("percent", "amount", "fixed"):
            raise ValueError("invalid offer kind")
        await self.set_setting(SETTING_OFFER_ENABLED, "1")
        await self.set_setting(SETTING_OFFER_KIND, kind)
        await self.set_setting(SETTING_OFFER_VALUE, str(value))

    async def clear_global_offer(self) -> None:
        await self.set_setting(SETTING_OFFER_ENABLED, "0")
        await self.set_setting(SETTING_OFFER_KIND, "none")
        await self.set_setting(SETTING_OFFER_VALUE, "0")

    async def resolve_price(self, base_price: int) -> int:
        from app.pricing import apply_offer

        return apply_offer(base_price, await self.get_offer_config())

    async def get_log_channel_id(self) -> int | None:
        raw = await self.get_setting(SETTING_LOG_CHANNEL)
    ...
    async def set_log_channel_id(self, chat_id: int | None) -> None:
        if chat_id is None:
            await self.set_setting(SETTING_LOG_CHANNEL, "0")
        else:
            await self.set_setting(SETTING_LOG_CHANNEL, str(chat_id))

    async def is_required_channel_turned_off(self) -> bool:
        return (await self.get_setting(SETTING_REQUIRED_CHANNEL_OFF) or "0") == "1"

    async def get_required_channel_id(self) -> int | None:
        raw = await self.get_setting(SETTING_REQUIRED_CHANNEL)
        if not raw or raw in ("0", "-"):
            return None
        try:
            return int(raw)
    ...
    async def get_required_channel_link(self) -> str | None:
        raw = (await self.get_setting(SETTING_REQUIRED_CHANNEL_LINK) or "").strip()
        return raw or None

    async def get_required_channel_title(self) -> str | None:
        raw = (await self.get_setting(SETTING_REQUIRED_CHANNEL_TITLE) or "").strip()
        return raw or None

    async def set_required_channel_link(self, link: str | None) -> None:
        await self.set_setting(SETTING_REQUIRED_CHANNEL_LINK, (link or "").strip())
    ...
    async def set_required_channel(
        ...
        *,
        ...
        if chat_id is None:
            await self.set_setting(SETTING_REQUIRED_CHANNEL, "0")
            await self.set_setting(SETTING_REQUIRED_CHANNEL_LINK, "")
            await self.set_setting(SETTING_REQUIRED_CHANNEL_TITLE, "")
            await self.set_setting(SETTING_REQUIRED_CHANNEL_OFF, "1")
            return
        await self.set_setting(SETTING_REQUIRED_CHANNEL_OFF, "0")
        await self.set_setting(SETTING_REQUIRED_CHANNEL, str(chat_id))
        await self.set_setting(SETTING_REQUIRED_CHANNEL_LINK, (link or "").strip())
        if title is not None:
            await self.set_setting(SETTING_REQUIRED_CHANNEL_TITLE, title)

    async def add_service_package(
        self,
        location_id: int,
        volume_gb: int,
        duration_days: int,
    ...
    async def remove_service_package(self, package_id: int) -> bool:
        async with self._cursor() as cur:
            await cur.execute("DELETE FROM service_packages WHERE id = ?", (package_id,))
            return cur.rowcount > 0
    ...
    async def update_service_package(
        ...
        if volume_gb <= 0 or duration_days <= 0 or price < 0:
            return False, "invalid"
        pkg = await self.get_service_package(package_id)
        if pkg is None:
            return False, "not_found"
        loc = await self.get_location(pkg.location_id)
        if loc is None:
            return False, "not_found"
        if loc.is_test:
    ...
    async def get_service_package(self, package_id: int) -> ServicePackage | None:
        async with self._cursor() as cur:
            await cur.execute("SELECT * FROM service_packages WHERE id = ?", (package_id,))
            row = await cur.fetchone()
            if row is None:
                return None
    ...
    async def list_service_packages(
        ...
        async with self._cursor() as cur:
            ...
            rows = await cur.fetchall()
            return [_row_to_service_package(r) for r in rows]

    async def list_all_service_packages(self) -> list[ServicePackage]:
        async with self._cursor() as cur:
            await cur.execute(
    ...
    async def remove_duration_preset(self, days: int) -> tuple[bool, str]:
        presets = await self.get_duration_presets()
    ...
    async def get_pricing_for_location(self, location_id: int) -> tuple[int, int, int]:
        """Resolved pricing for a location (custom or global fallback per field)."""
        g_base, g_per_gb, g_per_day = await self.get_pricing()
        loc = await self.get_location(location_id)
        if loc is None:
    ...
    async def add_location(
        ...
        *,
        ...
        if price_base is None:
            price_base, price_per_gb, price_per_day = await self.get_pricing()
        async with self._cursor() as cur:
            await cur.execute(
    ...
    async def replace_test_location(
        self,
        *,
        name: str,
        base_url: str,
        ...
        async with self._cursor() as cur:
            await cur.execute("SELECT id FROM locations WHERE is_test = 1")
            rows = await cur.fetchall()
            for row in rows:
                old_id = int(row["id"])
                await cur.execute(
                ...
                if order_row and int(order_row["c"]) > 0:
                    await cur.execute(
                        "UPDATE locations SET enabled = 0 WHERE id = ?", (old_id,)
                    )
                else:
                    await cur.execute("DELETE FROM locations WHERE id = ?", (old_id,))
    ...
    async def get_test_location(self) -> Location | None:
        async with self._cursor() as cur:
            await cur.execute(
    ...
    async def is_test_feature_enabled(self) -> bool:
        return await self.get_setting(SETTING_TEST_ENABLED, "0") == "1"

    async def set_test_feature_enabled(self, enabled: bool) -> None:
        await self.set_setting(SETTING_TEST_ENABLED, "1" if enabled else "0")

    async def user_has_claimed_test(self, user_id: int) -> bool:
        async with self._cursor() as cur:
            await cur.execute(
    ...
    async def clear_test_clients(self) -> int:
        async with self._cursor() as cur:
            await cur.execute("DELETE FROM orders WHERE is_test = 1")
            return cur.rowcount

    async def set_location_pricing(
        self,
        location_id: int,
        *,
        price_base: int | None,
    ...
    async def set_location_sub_url_template(
    ...
    async def set_location_config_buttons(self, location_id: int, buttons: list[dict[str, Any]]) -> bool:
        async with self._cursor() as cur:
            await cur.execute(
    ...
    async def remove_location(self, location_id: int) -> str:
        ...
        """
        async with self._cursor() as cur:
            await cur.execute("SELECT id FROM locations WHERE id = ?", (location_id,))
            row = await cur.fetchone()
            if row is None:
                return "not_found"
            await cur.execute(
            ...
            order_row = await cur.fetchone()
            has_orders = order_row and int(order_row["c"]) > 0
            if has_orders:
                await cur.execute(
                    "UPDATE locations SET enabled = 0 WHERE id = ?", (location_id,)
                )
                return "disabled"
            await cur.execute("DELETE FROM locations WHERE id = ?", (location_id,))
            return "deleted"

    async def set_location_enabled(self, location_id: int, enabled: bool) -> bool:
        async with self._cursor() as cur:
            await cur.execute(
    ...
    async def set_location_purchase_enabled(
    ...
    async def count_orders_for_location(self, location_id: int) -> int:
        async with self._cursor() as cur:
            await cur.execute(
                "SELECT COUNT(*) AS c FROM orders WHERE location_id = ?",
                (location_id,),
    ...
    async def purge_location(self, location_id: int) -> str:
        ...
        async with self._cursor() as cur:
            ...
            if row is None:
                return "not_found"
            await cur.execute("DELETE FROM orders   WHERE location_id = ?", (location_id,))
            await cur.execute("DELETE FROM locations WHERE id = ?",           (location_id,))
            return "purged"

    async def get_location(self, location_id: int) -> Location | None:
        async with self._cursor() as cur:
            await cur.execute("SELECT * FROM locations WHERE id = ?", (location_id,))
            row = await cur.fetchone()
            return _row_to_location(row) if row else None
    ...
    async def list_locations(
        ...
        *,
        ...
        async with self._cursor() as cur:
            await cur.execute(f"SELECT * FROM locations {where} ORDER BY id")
            rows = await cur.fetchall()
    ...
    async def set_order_screenshot(
    ...
    async def add_admin_receipt_message(
    ...
    async def get_admin_receipt_refs(self, order_id: int) -> dict[int, int]:
        order = await self.get_order(order_id)
        if order is None:
            return {}
        raw = order["admin_receipt_refs"]
    ...
    async def set_order_provisioned(
        self,
        order_id: int,
        email: str,
        sub_id: str | None,
    ...
    async def list_user_orders(self, user_id: int, limit: int = 50) -> list[sqlite3.Row]:
    ...
    async def set_order_nickname(self, order_id: int, nickname: str | None) -> bool:
        async with self._cursor() as cur:
            await cur.execute(
    ...
    async def count_orders(self) -> int:
        async with self._cursor() as cur:
            await cur.execute("SELECT COUNT(*) AS c FROM orders")
            row = await cur.fetchone()
            return int(row["c"]) if row else 0
    ...
    async def count_orders_by_status(self, status: str) -> int:
        async with self._cursor() as cur:
            if status == "provisioned":
                ...
                await cur.execute("SELECT COUNT(*) AS c FROM orders WHERE status = ?", (status,))
            row = await cur.fetchone()
            return int(row["c"]) if row else 0

    async def pending_orders(self, limit: int = 20) -> list[sqlite3.Row]:
        async with self._cursor() as cur:
            await cur.execute(
    ...
    async def list_provisioned_orders(
    ...
    async def detach_test_order_from_panel(self, order_id: int) -> bool:
    ...
    async def set_order_warning_sent(
        self,
        order_id: int,
        *,
        ...
        async with self._cursor() as cur:
            if traffic_warning:
                await cur.execute(
                    "UPDATE orders SET traffic_warning_sent = 1 WHERE id = ?",
                    (order_id,),
                )
            if expiry_warning:
                await cur.execute(
                    "UPDATE orders SET expiry_warning_sent = 1 WHERE id = ?",
                    (order_id,),
                )

    async def delete_order(self, order_id: int) -> bool:
        async with self._cursor() as cur:
            await cur.execute("DELETE FROM orders WHERE id = ?", (order_id,))
            return cur.rowcount > 0

    async def delete_orders_by_status(self, status: str) -> int:
        async with self._cursor() as cur:
            await cur.execute("DELETE FROM orders WHERE status = ?", (status,))
            return cur.rowcount
    ...
    async def create_ticket(self, user_id: int, message: str) -> int:
        ...
        await self.create_ticket_reply(ticket_id, "user", message)
        return ticket_id

    async def get_ticket(self, ticket_id: int) -> sqlite3.Row | None:
        async with self._cursor() as cur:
            await cur.execute("SELECT * FROM tickets WHERE id = ?", (ticket_id,))
            return await cur.fetchone()
    ...
    async def count_tickets(self) -> int:
        async with self._cursor() as cur:
            await cur.execute("SELECT COUNT(*) AS c FROM tickets")
            row = await cur.fetchone()
            return int(row["c"]) if row else 0

    # ---------- ticket replies ----------
    async def create_ticket_reply(self, ticket_id: int, sender: str, message: str) -> int:
        async with self._cursor() as cur:
            await cur.execute(
    ...
    async def list_ticket_replies(self, ticket_id: int) -> list[sqlite3.Row]:
        async with self._cursor() as cur:
            await cur.execute(
