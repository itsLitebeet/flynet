...
async def service_packages(packages: list, db) -> InlineKeyboardMarkup:
    """One plan per row: two buttons (volume + term/price), same callback each."""
    rows: list[list[InlineKeyboardButton]] = []
    for pkg in packages:
        cb = f"{CB_SVC_PREFIX}{pkg.id}"
        final = await db.resolve_price(int(pkg.price))
        rows.append([
            InlineKeyboardButton(
...
# ---------- admin reply keyboard ----------
async def _admin_perm(user_id: int, perm: str, settings, db) -> bool:
    from app.admin_perms import has_permission

    return await has_permission(user_id, perm, settings, db)


async def admin_reply_keyboard(user_id: int, settings, db) -> ReplyKeyboardMarkup:
    ...
    if await _admin_perm(user_id, SETTINGS, settings, db) or await _admin_perm(
...
# ---------- admin panel inline ----------
async def admin_home_inline(user_id: int, settings, db) -> InlineKeyboardMarkup:
    from app.admin_perms import (
        CUSTOMERS,
        LOCATIONS,
        OFFER,
    ...
    if await _admin_perm(user_id, ORDERS_MANAGE, settings, db):
        order_row.append(
            InlineKeyboardButton(
    ...
    ) or await _admin_perm(user_id, OFFER, settings, db):
        r3.append(
            InlineKeyboardButton(
...
async def admin_pending_footer(user_id: int, settings, db) -> InlineKeyboardMarkup:
...
async def admin_roles_keyboard(settings, db) -> InlineKeyboardMarkup:
    ...
    rows: list[list[InlineKeyboardButton]] = []
    for uid, _role in await db.list_staff_roles(settings.admin_ids):
